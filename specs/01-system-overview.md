...
| Embedding      | Vertex AI — Gemini `text-embedding-004`| Google Cloud (asia-southeast1)           |
...
| Vector Embedding                | A fixed-dimension numerical representation of text generated by a machine learning model (Gemini `text-embedding-004`), enabling semantic similarity search via cosine distance. |
...
| Vertex AI                       | Google Cloud's machine learning platform. Used here to access the Gemini `text-embedding-004` model for generating text embeddings. See: https://cloud.google.com/vertex-ai/generative-ai/docs/embeddings/get-text-embeddings |
...
- **Vertex AI**: Gemini `text-embedding-004` model in `asia-southeast1` for generating text embeddings
