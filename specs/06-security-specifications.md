...
- The only personal-adjacent data collected is: truncated IP address, browser name/version, pages visited, referrer URL, connection speed information, approximate country (derived from truncated IP via embedded MaxMind GeoLite2-Country database — country-level only, accuracy reduced by IP truncation) (CLR-133), and timestamps.
