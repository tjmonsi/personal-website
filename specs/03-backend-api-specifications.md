...
3. **Ban status check** — Look up the client IP in the LRU cache (then Firestore on cache miss) for active bans. Return `403` or `404` as appropriate. (Note: HTTP `429` responses originate from Cloud Armor at the infrastructure level before requests reach Cloud Run; the Go backend's ban check only returns `403` for 30-day bans and `404` for 90-day/indefinite bans.)
...
- THE SYSTEM SHALL only return the following HTTP status codes to clients:
  ...
  - `429` — Too Many Requests (generated by Cloud Armor at the load balancer level when rate limits are exceeded; not produced by the Go backend application)
