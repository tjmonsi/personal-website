...
- **Rewrite rules** (`firebase.json`):
  ...
  - All non-asset routes SHALL be rewritten to the Firebase Function (`server`) for SPA shell serving.
  ...
  - The `/sitemap.xml` rewrite routes to the Cloud Run backend which generates the sitemap. The `**` catch-all rewrite routes all other non-asset routes to the Firebase Function for SPA shell serving.
  ...
  > **Note**: The Nuxt 4 Firebase preset (`nuxt build --preset=firebase`) auto-generates the SPA catch-all rewrite (`{"source": "**", "function": "server"}`). The `firebase.json` shown above represents the complete rewrite configuration including both the manually added sitemap rule and the Nuxt-generated catch-all. Rewrite rules are evaluated in order — the specific `/sitemap.xml` rule takes precedence over the `**` catch-all.
...
- Cloud Logging log sinks — BigQuery sinks (INFRA-010a–010f) + Pub/Sub sink for rate-limit events (INFRA-008c)
...
Six Cloud Logging log sinks route logs to dedicated BigQuery tables within the `website_logs` dataset:
...
##### INFRA-010f: Cloud Functions Error Logs

| Setting          | Value                                              |
| ---------------- | -------------------------------------------------- |
| Sink name        | `sink-cloud-functions-errors`                      |
| Destination      | BigQuery table `website_logs.cloud_functions_error_logs` |
| Filter           | `resource.type="cloud_run_revision" AND resource.labels.service_name!="website-api" AND severity>=ERROR` |
| Purpose          | Cloud Functions error trend analysis — sitemap generation, rate-limit log processing, offender cleanup, embedding sync failures |

> **Note**: Cloud Functions Gen 2 run on Cloud Run infrastructure (`resource.type="cloud_run_revision"`). This sink captures ERROR-level logs from all Cloud Run revisions EXCEPT the Go backend (`website-api`), which is covered by INFRA-010d. The `service_name` exclusion filter is the inverse of INFRA-010d's inclusion filter, ensuring no overlap between the two sinks.
...
- This 2-year retention applies to 5 of the 6 tables. The `cloud_armor_lb_logs` table overrides this to 90 days (see INFRA-010b). No table retains data indefinitely.
...
- **AC-INFRA-021**: Given the BigQuery log sinks (INFRA-010a–010f), when Cloud Functions Gen 2 emit ERROR-level logs, then those errors are captured in the `cloud_functions_error_logs` table (INFRA-010f) and NOT in the `backend_error_logs` table (INFRA-010d).
