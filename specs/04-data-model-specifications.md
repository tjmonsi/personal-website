...
| `vector`       | double[]   | Yes      | 768-dimensional embedding vector from Gemini `gemini-embedding-001` (L2-normalized) |
...
| `embedding`          | vector(768)| Yes      | 768-dimensional embedding vector (Gemini `gemini-embedding-001`, task type: `RETRIEVAL_DOCUMENT`, L2-normalized) |
...
- Vector embeddings are generated by the `sync-article-embeddings` Cloud Function (INFRA-014) using Gemini `gemini-embedding-001` with `task_type=RETRIEVAL_DOCUMENT` and `output_dimensionality=768`. Vectors are L2-normalized before storage.
