...
**Answer**
- Option C if the goal is to honor the `?page=N` parameter as the starting point. If `page` attribute is not there, do A. If `page` is there, do C. This allows deep linking to a specific page while still supporting infinite scroll on mobile. Scrolling up when it is in first page or no `page` parameter should just refresh the whole site.
...
**Answer**
- (C) Both A and B — show the complete firebase.json and note it's auto-generated by Nuxt. This provides clarity and a complete example while acknowledging the role of the Nuxt preset in generating the correct configuration. It should go to the Cloud Run backend and Cloud Run backend will generate the `sitemap.xml`
...
**Answer**
- Use A. This provides a clear, documented process for lifting indefinite bans without adding unnecessary complexity. It also clarifies that while the ban is "indefinite" in terms of the escalation ladder, it can be manually lifted if needed, which is important for handling edge cases like shared IPs or false positives.
...
**Answer**
- Use A. This provides a dedicated sink for Cloud Functions errors without diluting the backend error logs. It allows for easier monitoring and alerting on CF-specific issues while keeping backend API errors separate for clarity. The filter ensures we capture CF errors without including the website-api service logs, which are already covered in INFRA-010d. This approach maintains organized log management and supports proactive issue detection in critical backend processes like embedding sync and rate-limit handling.
